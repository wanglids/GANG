...
Baking::sparse_interpolate_coefficients(const Tensor occlusion_coefficients, // [num_grid, d2]
...
Tensor Baking::SH_reconstruction(const Tensor coefficients, // [num_rays, d2, C]
...
void Baking::dialate_occlusion_ids(Tensor occlusion_ids) {
    CHECK_INPUT(occlusion_ids);
