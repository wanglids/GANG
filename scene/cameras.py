...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image,normal,albedo,roughness,
        ...
        if normal is not None:
            ...
            self.metal = metal.to(self.data_device) #* 0.6
            self.irradiance = irradiance.to(self.data_device) #* 0.25 
        else:
            self.normal = self.albedo = self.roughness = self.metal = self.irradiance = None
    
 
        if gt_alpha_mask is not None:
            self.mask = gt_alpha_mask.clamp(0.0, 1.0).to(self.data_device)
            ...
            if normal is not None:
                ...
                self.albedo *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
                self.roughness *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
                self.metal *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
                self.irradiance *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
