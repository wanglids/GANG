...
class GaussianModel:
    ...
    def __init__(self, 
        ...
        # SDF
        # self.SDF = SDF(in_channels=3, geom_feat_size_out=32, nr_iters_for_c2f=10000*1.0).to("cuda")
        ...
        self.roughness = torch.empty(0)
        self.matallic = torch.empty(0)
    ...
    def restore(self, model_args, training_args= None):
        ...
        if self.is_pbr:
            ...
            if not self.normal_detal and len(model_args)>18:
                (mlp_albedo,
                mlp_matallic,
                mlp_roughness
                ) = model_args[18:]
            ...
            elif self.normal_detal and len(model_args)>20:
                (
                mlp_albedo,
                mlp_matallic,
                mlp_roughness,
            ...
            if self.normal_detal and len(model_args)>18:
                (mlp_normal1,
                mlp_normal2) = model_args[18:20]
                self.mlp_normal1.load_state_dict(mlp_normal1)
                self.mlp_normal2.load_state_dict(mlp_normal2)
        try:    
    ...
    def create_from_pcd(self, points, spatial_lr_scale, logger=None):
        ...
        self.centroid = torch.mean(points, axis=0)
    ...
    def training_setup(self, training_args):
        ...
        if self.is_pbr:
            l.append({'params': self.mlp_albedo.parameters(), 'lr': training_args.mlp_albedo_lr_init,
                      "name": "mlp_albedo"})
            l.append({'params': self.mlp_matallic.parameters(), 'lr': training_args.mlp_matallic_lr_init,
                     "name": "mlp_matallic"})
            l.append({'params': self.mlp_roughness.parameters(), 'lr': training_args.mlp_roughness_lr_init,
                      "name": "mlp_roughness"})
        ...
        if self.is_pbr:
            ...
            self.mlp_albedo_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_albedo_lr_init,
                                                       lr_final=training_args.mlp_albedo_lr_final,
                                                       lr_delay_mult=training_args.mlp_albedo_delay_mult,
                                                       max_steps=training_args.mlp_albedo_lr_max_steps)

            self.mlp_matallic_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_matallic_lr_init,
                                                       lr_final=training_args.mlp_matallic_lr_final,
                                                       lr_delay_mult=training_args.mlp_matallic_delay_mult,
                                                       max_steps=training_args.mlp_matallic_lr_max_steps)
            ...
            self.mlp_roughness_scheduler_args = get_expon_lr_func(lr_init=training_args.mlp_roughness_lr_init,
                                                                ...
                                                                lr_delay_mult=training_args.mlp_roughness_delay_mult,
    ...
    def load_ply_sparse_gaussian(self, path):
        ...
        # self.centroid = torch.mean(anchor, axis=0)
    ...
    def training_statis(self, viewspace_point_tensor, opacity, update_filter, offset_selection_mask, anchor_visible_mask):
        ...
        # print(combined_mask.shape,anchor_visible_mask.shape,offset_selection_mask.shape)
    ...
    def muti_plane_pruning(self, num=10,std = 2,planer_numer=16):
        ...
        depth_mask = torch.zeros(self.get_anchor.shape[0]).cuda()
        for i in range(planer_numer):
            depth_mask[self.get_anchor[:, 2] <= (depth_min + (1 + i) * depth_vector)] += 1

        mask_point_temp = torch.logical_or(torch.zeros(self.get_anchor.shape[0]), torch.zeros(self.get_anchor.shape[0])).cuda()
        ...
        for i in range(planer_numer):
            ...
            pcd_vector.points = o3d.utility.Vector3dVector(self.get_anchor[muti_mask==1].detach().cpu().numpy())
            cl, ind = pcd_vector.remove_statistical_outlier(num, std)
            mask_t = torch.zeros(muti_mask[muti_mask==1].shape[0]).cuda()
            mask_t[ind] = 1
    ...
    def adjust_anchor(self, iteration, check_interval=100, success_threshold=0.8, grad_threshold=0.0002, update_ratio=0.5, extra_ratio=4.0, extra_up=0.25, min_opacity=0.005):
        ...
        if prune_mask.shape[0]>0:
            self.prune_anchor(prune_mask)  # prune 为 Ture，保留为 False
    ...
    def save_mlp_checkpoints(self, path, mode = 'unite'):#split or unite
        ...
        if mode == 'split':
            ...
            if self.normal_detal:
                normal1_mlp = torch.jit.trace(self.mlp_normal1, (torch.rand(1, self.feat_dim + self.view_dim + self.color_dist_dim + self.appearance_dim + self.level_dim).cuda()))
                normal1_mlp.save(os.path.join(path, 'normal1_mlp.pt'))

                normal2_mlp = torch.jit.trace(self.mlp_normal2, (torch.rand(1, self.feat_dim + self.view_dim + self.color_dist_dim + self.appearance_dim + self.level_dim).cuda()))
            ...
            if self.is_pbr:
                albedo_mlp = torch.jit.trace(self.mlp_albedo, (torch.rand(1, self.feat_dim + self.view_dim + self.color_dist_dim + self.appearance_dim + self.level_dim).cuda()))
                albedo_mlp.save(os.path.join(path, 'albedo_mlp.pt'))
                roughness_mlp = torch.jit.trace(self.mlp_roughness, (torch.rand(1, self.feat_dim+self.view_dim+self.opacity_dist_dim+self.level_dim).cuda()))
                roughness_mlp.save(os.path.join(path, 'roughness_mlp.pt'))
                mate_mlp = torch.jit.trace(self.mlp_matallic, (
                    torch.rand(1, self.feat_dim + self.view_dim + self.opacity_dist_dim + self.level_dim).cuda()))
    ...
    def load_mlp_checkpoints(self, path, mode = 'unite'):#split or unite
        if mode == 'split':
            ...
            # self.SDF = torch.jit.load(os.path.join(path, 'sdf_mlp.pt')).cuda()
            ...
            if self.is_pbr:
                self.mlp_albedo = torch.jit.load(os.path.join(path, 'albedo_mlp.pt')).cuda()
                self.mlp_roughness = torch.jit.load(os.path.join(path,'roughness_mlp.pt')).cuda()
                self.mlp_matallic = torch.jit.load(os.path.join(path,"matallic_mlp.pt")).cuda()
    ...
    def position_normal(self,points):
        if self.centroid is None:
            self.centroid = torch.mean(self._anchor, axis=0)
        points_centered = points - self.centroid
        scale = torch.max(torch.linalg.norm(points_centered, axis=1)) 
